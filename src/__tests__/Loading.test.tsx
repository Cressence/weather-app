...
const server = setupServer(
    rest.get(`${process.env.REACT_APP_API_URL}`, (req, res, ctx) => {
        return res(ctx.status(404), ctx.json({message: 'city not found'}));
    }),
);
...
describe('<Loading />: Loading Screen', () => {
    ...
    it('shows loader while data is loading', () => {
        const loadingText = screen.findByText('Almost there');
        expect(loadingText).toBeTruthy();
    });

    it('displays error if unknown city is sent', () => {
        server.use(
            rest.get(`${process.env.REACT_APP_API_URL}`, (req, res, ctx) => {
                return res(ctx.status(404), ctx.json({message: 'city not found'}));
            })
        ...
        const loadingText = screen.queryByText('Almost there');
        // const errorText = screen.queryByTestId('error-text');
